...
const apiClient: AxiosInstance = axios.create({
  ...
  withCredentials: false, // Set to false - we're using Authorization header, not cookies
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response) {
      ...
      console.error('Please ensure your backend server is running on port 8000');
